...
- Filtros rápidos (Todos, Vencen Hoy, Próximos 7 días, Pendientes, Vencidos, Pagados Hoy)
...
- Registro de pago con modal especializado (fecha, método, referencia, notas)
- Generación de comprobantes PDF con datos completos
- Dashboard con 4 tarjetas de métricas
- 6 filtros rápidos para acceso inmediato
- Modal de filtros avanzados con checkboxes múltiples
- Indicadores visuales de urgencia (9 niveles según días restantes)
- Badges de estado con colores (verde/amarillo/rojo)
- Click en recibo pendiente abre modal de pago
- Click en recibo pagado genera PDF automáticamente
- Formato inteligente de moneda con abreviaciones

== Flujo de Usuario

1. Usuario navega al módulo de Recibos
2. Ve dashboard con métricas: Por Cobrar, Cobrado, Vencidos, Total
3. Puede usar filtros rápidos (Vencen Hoy, Próximos 7 días, etc.)
4. Busca recibos por número, póliza, cliente o aseguradora
5. Hace click en recibo pendiente → Se abre modal de registro de pago
6. Llena método de pago, referencia, notas
7. Opcionalmente marca "Generar PDF" (checked por default)
8. Confirma pago → Estado cambia a "Pagado" y se genera PDF
9. Puede abrir el PDF generado o descartarlo
10. Click en recibo pagado genera PDF directamente

== Reglas de Negocio

*RN-REC-001:* Los recibos se pueden crear manualmente o generar automáticamente
*RN-REC-002:* El monto del recibo debe ser > 0
*RN-REC-003:* Al registrar pago se requiere fecha y método de pago
*RN-REC-004:* Un recibo pagado puede revertirse a pendiente
*RN-REC-005:* Los recibos vencidos son aquellos con fecha_corte < hoy y estado != pagado
*RN-REC-006:* El PDF incluye datos del recibo, pago, cliente y póliza
*RN-REC-007:* Los PDFs se guardan en ~/Documents/Comprobantes_Recibos/
*RN-REC-008:* El número de recibo es opcional, se autogenera si no se provee

= Casos de Prueba

== Categoría 1: CRUD Básico
