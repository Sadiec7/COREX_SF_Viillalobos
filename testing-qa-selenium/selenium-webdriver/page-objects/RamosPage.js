// RamosPage.js - Page Object para el módulo de Ramos

const { By } = require('selenium-webdriver');
const BasePage = require('./BasePage');
const { waitForVisible } = require('../helpers/wait-helpers');

class RamosPage extends BasePage {
  constructor(driver) {
    super(driver);

    this.locators = {
      searchInput: By.id('searchRamos'),
      btnAddRamo: By.id('btnAddRamo'),
      tableBody: By.id('ramosTableBody'),
      emptyState: By.id('emptyState'),
      modal: By.id('modalRamo'),
      ...
      btnSubmitForm: By.css('#formRamo button[type="submit"]')
    };
  }
  ...
  async closeAllModals() {
    try {
      for (let i = 0; i < 3; i++) {
        try {
          const modal = await this.driver.findElement(By.css('.modal.active, #modalRamo.active'));
  ...
  async navigateToRamos() {
    console.log('📍 Navegando a Ramos...');
    ...
    const navLink = By.css('a[data-view="ramos"]');
  ...
  async openNewModal() {
    ...
    await this.clickWithRetry(this.locators.btnAddRamo, 3, 1000);
  ...
  async create(data) {
    ...
    try {
      await this.driver.executeScript(`
        const modal = document.getElementById('modalRamo');
