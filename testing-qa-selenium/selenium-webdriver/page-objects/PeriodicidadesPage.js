// PeriodicidadesPage.js - Page Object para el módulo de Periodicidades

const { By } = require('selenium-webdriver');
const BasePage = require('./BasePage');
const { waitForVisible } = require('../helpers/wait-helpers');

class PeriodicidadesPage extends BasePage {
  constructor(driver) {
    super(driver);

    this.locators = {
      searchInput: By.id('searchPeriodicidades'),
      btnAddPeriodicidad: By.id('btnAddPeriodicidad'),
      tableBody: By.id('periodicidadesTableBody'),
      emptyState: By.id('emptyState'),
      modal: By.id('modalPeriodicidad'),
      modalTitle: By.id('modalTitle'),
      btnCloseModal: By.id('btnCloseModal'),
      form: By.id('formPeriodicidad'),
      inputNombre: By.id('inputNombre'),
      inputMeses: By.id('inputMeses'),
      inputDiasAnticipacion: By.id('inputDiasAnticipacionAlerta'),
      btnCancelForm: By.id('btnCancelForm'),
      btnSubmitForm: By.css('#formPeriodicidad button[type="submit"]')
    };
  }
  ...
  async closeAllModals() {
    try {
      for (let i = 0; i < 3; i++) {
        try {
          const modal = await this.driver.findElement(By.css('.modal.active, #modalPeriodicidad.active'));
  ...
  async navigateToPeriodicidades() {
    console.log('📍 Navegando a Periodicidades...');
    ...
    const navLink = By.css('a[data-view="periodicidades"]');
  ...
  async openNewModal() {
    ...
    await this.clickWithRetry(this.locators.btnAddPeriodicidad, 3, 1000);
  ...
  async fillForm(data) {
    ...
    if (data.meses) {
      await this.type(this.locators.inputMeses, data.meses);
    }
    // dias_anticipacion_alerta no existe en el formulario, se omite
  }
  ...
  async create(data) {
    ...
    try {
      await this.driver.executeScript(`
        const modal = document.getElementById('modalPeriodicidad');
...
module.exports = PeriodicidadesPage;
